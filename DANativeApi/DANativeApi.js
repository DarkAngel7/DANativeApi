...
!(function(window, undefined) {
    ...
    function _callNativeApi(apiName, param) {
        DANativeApi._callbackId ++;
        DANativeApi._callbacks[DANativeApi._callbackId.toString()] = param.completion || function(res) {};
        param["callbackId"] = DANativeApi._callbackId;
        param = param && _convertFunctionToString(param) || {};

        if (!_verifyHasFunction(apiName)) return delete DANativeApi._callbacks[DANativeApi._callbackId.toString()];
    ...
    DANativeApi._callbackId = 0;
    DANativeApi._callbacks = {};
    ...
    DANativeApi.prototype = {
        ...
        callback: function(callbackId, param) {
            DANativeApi._callbacks[callbackId](param);
            delete DANativeApi._callbacks[callbackId];
        }
    };
...
// /**
//  * 用于Android端，设置是否正在刷新，iOS端无此Api，请不要调用
